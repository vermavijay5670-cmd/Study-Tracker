...
export function GlowCard({ accent, children, className = "", delay = 0, variant = "glow" }: GlowCardProps) {
  ...
  const rotateX = useSpring(useTransform(my, [0, 1], [7, -7]), { stiffness: 200, damping: 20, mass: 0.4 });
  const rotateY = useSpring(useTransform(mx, [0, 1], [-7, 7]), { stiffness: 200, damping: 20, mass: 0.4 });
